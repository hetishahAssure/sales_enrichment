// ── Prepare Requests ──────────────────────────────────────────────────────
// Runs once per company row. Builds the request bodies/queries for all four
// sources so the HTTP nodes stay simple. Reads either your real Google Sheet
// headers (Account Name, Website, LinkedIn, Billing State/Province) or the
...
const company = String(c["Account Name"] || c["company_name"] || "").trim();
const website = String(c["Website"] || c["company_website"] || "").trim();
const liUrl   = String(c["LinkedIn"] || c["company_linkedin_url"] || "").trim();
const state   = String(c["Billing State/Province"] || c["state"] || "").trim();
